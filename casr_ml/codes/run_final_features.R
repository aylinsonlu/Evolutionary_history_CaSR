...
library(xgboost)
library(PRROC)
source("./feature_selection_grid_search_cv.R")
source("./feature_selection_varImp_cv.R")

args <- commandArgs(trailingOnly = TRUE)
replication <- as.numeric(args[[1]])

id <- 2000 + replication
result_path <- "./results_final_feature"
set.seed(1903 * replication)

X_train <- read.csv(file = sprintf("./final_csv_for_feature_selection/%s_train.csv", id))
...
for (fold in 1:fold_count) {
  ...
  X_train2 <- read.csv(sprintf("./final_csv_for_feature_selection/%s_%s_train.csv", id, fold))
  y_train2 <- 2 * (X_train2$consequence == "loss-of-function") - 1
  X_train2 <- X_train2[,-c(1,86)]
  X_train2 <- scale(X_train2)
  
  X_test2 <- read.csv(sprintf("./final_csv_for_feature_selection/%s_%s_val.csv", id, fold))
